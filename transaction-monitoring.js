...
const pool = new web3.eth.Contract(abi, '0xB4e16d0168e52d35CaCD2c6185b44281Ec28C9Dc')
...
async function watchEvents() {
    ...
    console.log('Starting to listen at block:', blockNumber)
    var swaps = pool.events.Swap({
        fromBlock: blockNumber
    }, function(error, event){})
    .on('data', function(event){
        console.log('Block:',event.blockNumber)
        console.log('Transaction:',event.transactionHash)
        console.log('USDC In:',event.returnValues.amount0In);
        console.log('wETH In:',event.returnValues.amount1In);
        console.log('USDC Out:',event.returnValues.amount0Out);
        console.log('wETH Out:',event.returnValues.amount1Out);
    })
    .on('error', console.error);
}
*/

//----- Watching an ERC20 Token Contract...
async function watchEvents() {
    var blockNumber = await web3.eth.getBlockNumber()
    console.log('Starting to listen at block:', blockNumber)
    var swaps = baseToken.events.allEvents({
        fromBlock: blockNumber
    }, function(error, event){})
    .on('data', function(event){
        console.log('Block:',event.blockNumber)
        console.log('Event Type',event.event)
        console.log('Amount',web3.utils.fromWei(event.returnValues.wad, 'ether'))
    })
    .on('error', console.error);
}
